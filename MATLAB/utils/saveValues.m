function [] = saveValues(opt_RUN,bool_fit_repetitions_seperately,n_is,individual_IDs,group_str,sim_model_str,opt_model_str,noise_level)
...
if strcmp(opt_RUN,'test_inference_procedure')
        ...
        for om_id = 1: length(opt_model_str)
            ...
            for nl_id = 1:length(noise_level)
                for i_id = 1:length(individual_IDs)
                    target_dir = [ResultsFolderName,'/',opt_model_str{om_id},'/individual_',individual_IDs{i_id},'_',noise_level{nl_id},'_noise'];
                    cd(target_dir)
                    if om_id==1
    ...
    for om_id = 1:length(opt_model_str)
        ...
        for i_id = 1:length(individuals_str_simple)
            target_dir = ['./',ResultsFolderName,'/',opt_model_str{om_id},'/individual_',individuals_str_simple{i_id}];
            cd(target_dir)
            if om_id==1
                transformation_str =[];
                load(['WS_individual_',individuals_str_simple{i_id}],'parameters','transformation_str','opt','theta_test');
                cd(path);                
                opt.individuals = individuals_str_simple;
                opt.n_individuals = length(individuals_str_simple);
                rate_names_opt = opt.rates(~strwcmp(opt.rates,'x0_*'));
                if isempty(transformation_str)
                    transformation_str = opt.parTransformationStr;
                end
                opt.foldername = ResultsFolderName;
%                 if length(opt.subfoldername)>7
%                     opt.subfoldername = opt.subfoldername(1:7);
%                 end
            ...
            if i_id==length(individuals_str_simple)
                ...
                PAR_TEST_T = [];
                rate_names_test = [];
                save(['ws_parameters_',group_str,add_str_fit_repetitions_seperately,'.mat'],'par_min','par_max','CI_lower','CI_upper','PAR_OPT_T','PAR_TEST_T','rate_names_opt','rate_names_test','transformation_str','individuals_str');
                cd(path);
            end
