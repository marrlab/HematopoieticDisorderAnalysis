...
cd(CD);
    ...
    function [T_new] = adaptModelFile_AMICI_multipleIS(T,opt,iS_ID,rates_str,n_prolif, n_diff, n_death)
        ...
        state_str = [];
        for id=1:n_states
            state_str = [state_str, ' x', num2str(id)];
        end
        newline0 = {strcat('syms ',state_str)};
        T_new(find(strwcmp(T, '% create state syms*')==1)+1)=newline0;
        newline1 = {strcat(state_str)};
        ...
        if opt.fitInitialConds
            for id=1:mS+dS
                par_str{id} = strcat('x0_',num2str(id));
            end
        else
            par_str=[];
        end
        ...
        for CT_ID = 1:mS
            ...
            %% rates:
            if strcmp(opt.group,'CHIP')
                n_out_vec(CT_ID) = sum(strwcmp(rates_str,['*_',opt.modelStates{CT_ID},'*']))/2-sum(strwcmp(rates_str,['a_*_',opt.modelStates{CT_ID},'*','_h']));
            else
            ...
            if strcmp(opt.group,'CHIP')
                diff_rates_out_vec_h{CT_ID}=rates_str(find(strwcmp(rates_str,['*a_',opt.modelStates{CT_ID},'*_h'])));
                diff_rates_out_vec_m{CT_ID}=rates_str(find(strwcmp(rates_str,['*a_',opt.modelStates{CT_ID},'*_m'])));
            end
            [prolif_rate,diff_rates_i,diff_rates_o,death_rate] = getRateStrings(rates_str,opt.modelStates{CT_ID});
            if ~isempty(prolif_rate)
                if strcmp(opt.group,'CHIP')
                    prolif_rate_str = ['((1-p_mut)*',prolif_rate{1},'+p_mut*',prolif_rate{2},')','*',num2str(n_is)];
                else
                    prolif_rate_str = [prolif_rate,'*',num2str(n_is)];
                end
            else
                prolif_rate_str = [];
            end
            if ~isempty(death_rate)
                if strcmp(opt.group,'CHIP')
                    death_rate_str = ['+((1-p_mut)*',death_rate{1},'+p_mut*',death_rate{2},')','*',num2str(n_is)];
                else
            ...
            elseif iscell(diff_rates_o)
                if strcmp(opt.group,'CHIP')
                    diff_rates_o_h = {diff_rates_o{strwcmp(diff_rates_o,'*_h')==true}};
                    diff_rates_o_m = {diff_rates_o{strwcmp(diff_rates_o,'*_m')==true}};
                    if (length(diff_rates_o_h)==1 && length(diff_rates_o_m)==1)
                        DR_out_str = ['+((1-p_mut)*',diff_rates_o_h{1},'+p_mut*',diff_rates_o_m{1},')*',num2str(n_is)];
                    else
                        DR_out_h_str = ['(',diff_rates_o_h{1}];
                        DR_out_m_str = ['(',diff_rates_o_m{1}];
                        for id=2:length(diff_rates_o_h)
                            DR_out_h_str = ['+',DR_out_h_str,'+',diff_rates_o_h{id}];
                            DR_out_m_str = ['+',DR_out_m_str,'+',diff_rates_o_m{id}]; 
                        end
                        DR_out_h_str = [DR_out_h_str,')*',num2str(n_is),'*(1-p_mut)'];
                        DR_out_m_str = [DR_out_m_str,')*',num2str(n_is),'*(p_mut)'];
                        DR_out_str = [DR_out_h_str,'+',DR_out_m_str];
                    end
                else
                    DR_out_str = [diff_rates_o{1},'*',num2str(n_is)];
                    for id=2:length(diff_rates_o)
                        DR_out_str = ['+',DR_out_str,'+',diff_rates_o{id},'*',num2str(n_is)];
                    end
                end
            end
            for D_ID = 1:opt.n_divStates
                ...
                elseif iscell(diff_rates_i)
                    if strcmp(opt.group,'CHIP')
                        diff_rates_i_h = diff_rates_i{strwcmp(diff_rates_i,'*_h')==true};
                        diff_rates_i_m = diff_rates_i{strwcmp(diff_rates_i,'*_m')==true};
                        if (ischar(diff_rates_i_h) && ischar(diff_rates_i_m))
                            CT_ID_U = find(strcmp(opt.modelStates,diff_rates_i_h(3:5)));
                            [DivState_Vec_U,IS_Mat_U,Offset_U] = getIndices(opt.n_divStates,n_is,n_out_vec,CT_ID_U);
                            if size(IS_Mat_U,2)==boundary
                                source_state_D_ID = find(ismember(IS_Mat_U(:,1:end),[0,n_is,zeros(1,size(IS_Mat_U,2)-2)],'rows') & ismember(DivState_Vec_U,D_ID));
                            else
                                ID_rate_h = find(strcmp(diff_rates_out_vec{CT_ID_U},diff_rates_i_h));
                                source_state_D_ID = find(ismember(IS_Mat_U(:,1:end),[zeros(1,ID_rate_h),n_is,zeros(1,size(IS_Mat_U,2)-(ID_rate_h+1))],'rows') & ismember(DivState_Vec_U,D_ID));
                            end
                            DR_in_str = ['(',diff_rates_i_h,'*(1-p_mut)+',diff_rates_i_m,'*p_mut)*',num2str(n_is),'*x',num2str(Offset_U+source_state_D_ID)];
                        else
                            for j=1:length(diff_rates_i_h)
                                CT_ID_U_h = find(strcmp(opt.modelStates,diff_rates_i_h{j}(3:5)));
                                CT_ID_U_m = find(strcmp(opt.modelStates,diff_rates_i_m{j}(3:5)));
                                [DivState_Vec_U_h,IS_Mat_U_h,Offset_U_h] = getIndices(opt.n_divStates,n_is,n_out_vec,CT_ID_U_h);
                                [DivState_Vec_U_m,IS_Mat_U_m,Offset_U_m] = getIndices(opt.n_divStates,n_is,n_out_vec,CT_ID_U_m);
                                if size(IS_Mat_U_h,2)==boundary
                                    source_state_D_ID_h = find(ismember(IS_Mat_U_h(:,1:end),[0,n_is,zeros(1,size(IS_Mat_U_h,2)-2)],'rows') & ismember(DivState_Vec_U_h,D_ID));
                                else
                                    ID_rate_h = find(strcmp(diff_rates_out_vec_h{CT_ID_U_h},diff_rates_i_h{j}));
                                    source_state_D_ID_h = find(ismember(IS_Mat_U_h(:,1:end),[zeros(1,ID_rate_h),n_is,zeros(1,size(IS_Mat_U_h,2)-(ID_rate_h+1))],'rows') & ismember(DivState_Vec_U_h,D_ID));
                                end
                                if size(IS_Mat_U_m,2)==boundary
                                    source_state_D_ID_m = find(ismember(IS_Mat_U_m(:,1:end),[0,n_is,zeros(1,size(IS_Mat_U_m,2)-2)],'rows') & ismember(DivState_Vec_U_m,D_ID));
                                else
                                    ID_rate_m = find(strcmp(diff_rates_out_vec_m{CT_ID_U_m},diff_rates_i_m{j}));
                                    source_state_D_ID_m = find(ismember(IS_Mat_U_m(:,1:end),[zeros(1,ID_rate_m),n_is,zeros(1,size(IS_Mat_U_m,2)-(ID_rate_m+1))],'rows') & ismember(DivState_Vec_U_m,D_ID));
                                end
                                if j==1
                                    DR_in_h_str = ['(1-p_mut)*',num2str(n_is),'*(',diff_rates_i_h{j},'*x',num2str(Offset_U_h+source_state_D_ID_h)];
                                    DR_in_m_str = ['p_mut*',num2str(n_is),'*(',diff_rates_i_m{j},'*x',num2str(Offset_U_m+source_state_D_ID_m)];
                                else
                                    DR_in_h_str = [DR_in_h_str,'+', diff_rates_i_h{j},'*x',num2str(Offset_U_h+source_state_D_ID_h)];
                                    DR_in_m_str = [DR_in_m_str,'+', diff_rates_i_m{j},'*x',num2str(Offset_U_m+source_state_D_ID_m)];
                                end
                                DR_in_str = [DR_in_h_str,')+',DR_in_m_str,')'];
                            end
                        end
                    else
                        for j=1:length(diff_rates_i)
                            CT_ID_U = find(strcmp(opt.modelStates,diff_rates_i{j}(3:5)));
                            [DivState_Vec_U,IS_Mat_U,Offset_U] = getIndices(opt.n_divStates,n_is,n_out_vec,CT_ID_U);
                            if size(IS_Mat_U,2)==boundary
                                source_state_D_ID = find(ismember(IS_Mat_U(:,1:end),[0,n_is,zeros(1,size(IS_Mat_U,2)-2)],'rows') & ismember(DivState_Vec_U,D_ID));
                            else
                                ID_rate = find(strcmp(diff_rates_out_vec{CT_ID_U},diff_rates_i{j}));
                                source_state_D_ID = find(ismember(IS_Mat_U(:,1:end),[zeros(1,ID_rate),n_is,zeros(1,size(IS_Mat_U,2)-(ID_rate+1))],'rows') & ismember(DivState_Vec_U,D_ID));
                            end
                            if j==1
                                DR_in_str = [diff_rates_i{j},'*',num2str(n_is),'*x',num2str(Offset_U+source_state_D_ID)];
                            else
                                DR_in_str = [DR_in_str,'+', diff_rates_i{j},'*',num2str(n_is),'*x',num2str(Offset_U+source_state_D_ID)];
                            end
                        end
                    end
                end
                ...
                if D_ID == 1 %DR_in_str-1/n(prolif+diff_o+death)*xi
                    newline3 = {['xdot(',num2str(EQ_ID),') =',DR_in_str, '-(',prolif_rate_str,DR_out_str,death_rate_str,')*x',num2str(EQ_ID),';']};
                else
                    if opt.modelAccumulateInLastState 
                        ...
                        if ~isempty(source_state_P1_ID)
                            Prolif_in_str_1 = ['+2*',prolif_rate_str,'*x',num2str(Offset+source_state_P1_ID)];
                        end
                        ...
                        if ~isempty(source_state_P2_ID)
                            Prolif_in_str_2 = ['+2*',prolif_rate_str,'*x',num2str(Offset+source_state_P2_ID)];
                        end
                    else
                        Prolif_in_str_1='';
                        Prolif_in_str_2='';
                    ...
                    if D_ID == opt.n_divStates %DR_in_str+Prolif_in_str_1-1/n(prolif+diff_o+death)*xi
                        newline3 = {['xdot(',num2str(EQ_ID),') =',DR_in_str,Prolif_in_str_1,Prolif_in_str_2,'-(',prolif_rate_str,DR_out_str,death_rate_str,')*x',num2str(EQ_ID),';']};
                    else
                        newline3 = {['xdot(',num2str(EQ_ID),') =',DR_in_str,Prolif_in_str_1,'-(',prolif_rate_str,DR_out_str,death_rate_str,')*x',num2str(EQ_ID),';']};
                    end
                end
                ...
                if ~isempty(prolif_rate)
                    for IS_ID = 1:n_is
                        ...
                        T_new(ID+EQ_ID) = {['xdot(',num2str(EQ_ID),') =','+',prolif_rate_str,'*(x',num2str(Offset+state_in),'-x',num2str(Offset+state_out),');']};
                        EQ_ID = EQ_ID+1;
                    end
                end
                ...
                elseif iscell(diff_rates_o)
                    if strcmp(opt.group,'CHIP')
                        if ischar(diff_rates_o_h) && ischar(diff_rates_o_m)
                           for IS_ID = 1:n_is
                                state_in = find(ismember(IS_Mat(:,1:end),[0,IS_ID-1,zeros(1,size(IS_Mat,2)-2)],'rows') & ismember(DivState_Vec,D_ID));
                                state_out = find(ismember(IS_Mat(:,1:end),[0,IS_ID,zeros(1,size(IS_Mat,2)-2)],'rows') & ismember(DivState_Vec,D_ID));
                                T_new(ID+EQ_ID) = {['xdot(',num2str(EQ_ID),') =((1-p_mut)*',diff_rates_o_h,'+p_mut*',diff_rates_o_m,')*',num2str(n_is),'*(x',num2str(Offset+state_in),'-x',num2str(Offset+state_out),');']};
                                EQ_ID = EQ_ID+1;
                            end
                        else
                            for j=1:length(diff_rates_o_h)
                                for IS_ID = 1:n_is
                                    state_in = find(ismember(IS_Mat(:,1:end),[zeros(1,j),IS_ID-1,zeros(1,size(IS_Mat,2)-(j+1))],'rows') & ismember(DivState_Vec,D_ID));
                                    state_out = find(ismember(IS_Mat(:,1:end),[zeros(1,j),IS_ID,zeros(1,size(IS_Mat,2)-(j+1))],'rows') & ismember(DivState_Vec,D_ID));
                                    T_new(ID+EQ_ID) = {['xdot(',num2str(EQ_ID),') =((1-p_mut)*',diff_rates_o_h{j},'+p_mut',diff_rates_o_h{j},')*',num2str(n_is),'*(x',num2str(Offset+state_in),'-x',num2str(Offset+state_out),');']};
                                    EQ_ID = EQ_ID+1;
                                end
                            end
                        end
                    else
                        for j=1:length(diff_rates_o)
                            for IS_ID = 1:n_is
                                state_in = find(ismember(IS_Mat(:,1:end),[zeros(1,j),IS_ID-1,zeros(1,size(IS_Mat,2)-(j+1))],'rows') & ismember(DivState_Vec,D_ID));
                                state_out = find(ismember(IS_Mat(:,1:end),[zeros(1,j),IS_ID,zeros(1,size(IS_Mat,2)-(j+1))],'rows') & ismember(DivState_Vec,D_ID));
                                T_new(ID+EQ_ID) = {['xdot(',num2str(EQ_ID),') =',diff_rates_o{j},'*',num2str(n_is),'*(x',num2str(Offset+state_in),'-x',num2str(Offset+state_out),');']};
                                EQ_ID = EQ_ID+1;
                            end
                        end
                    end
