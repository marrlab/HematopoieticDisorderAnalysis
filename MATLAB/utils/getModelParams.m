...
switch model
    ...
    case 'model_intersect_ABDGI'
        opt.rates = {'a_{HSC_{MPP}}', 'a_{HSC_{MEP}}', 'a_{HSC_{mat}}', 'b_{HSC}', 'g_{HSC}', 'a_{MPP_{CMP}}', 'a_{MPP_{MLP}}', 'b_{MPP}', 'g_{MPP}', 'a_{CMP_{MEP}}','a_{CMP_{GMP}}', 'b_{CMP}', 'g_{CMP}', 'a_{MLP}','a_{MLP_{GMP}}', 'b_{MLP}', 'a_{MEP_{mat}}', 'b_{MEP}', 'g_{MEP}', 'a_{GMP_{mat}}', 'b_{GMP}', 'g_{GMP}', 'b_{mat}', 'g_{mat}'};
    otherwise
...
if strcmp(opt.group,'CHIP')
    rates=opt.rates;
    opt.rates = [{'p_{mut}'},rates,rates];
    for r=2:length(opt.rates)
        if r<=(length(opt.rates)+1)/2
            opt.rates{r} = [opt.rates{r}(1:end-1),'_h}'];
        else
            opt.rates{r} = [opt.rates{r}(1:end-1),'_m}'];
        end
    end
end
...
function [P_test,rates_test_str] = loadTestPar(opt,option,model)
    ...
    path = [c_dir,'/results_fit_samples_',num2str(opt.n_divStates),'divs_',num2str(opt.n_intermediateStates(opt.iS_ID)),'iS',HO_str,'_',opt.noiseType];
    ...
    if ~isempty(path) && ~(exist(path, 'dir')==0) && strcmp(option,'calculateFromRealDataFit')
        cd(path);
        cd('../../data/');
        [individual_IDs,~] = sort(getIndividuals(opt.fileName,opt.group,true));
        last_individual = individual_IDs{length(individual_IDs)};
        cd(path);
        cd(['./individual_',last_individual]);
        if opt.fit_repetitions_seperately==true
            load(['ws_parameters_',opt.group,'_rep_sep.mat'])
        else
            load(['ws_parameters_',opt.group,'.mat'])
        end
    ...
    cd(c_dir)
    if strcmp(opt.group,'CHIP')
        P_test = [0.5, P_test, P_test + unifrnd(-P_test/2,P_test/2)];
        rates = rates_test_str;
        rates_test_str = [{'p_{mut}'},rates(1:end-1),rates];
        for r=2:length(rates_test_str)-1
            if r<=(length(rates_test_str)+1)/2
                rates_test_str{r} = [rates_test_str{r}(1:end-1),'_h}'];
            else
                rates_test_str{r} = [rates_test_str{r}(1:end-1),'_m}'];
            end
        end
    end
end