...
if ~isempty(idx_ndcn)
    ...
    for i=1:length(idx_ndcn)
        str = [DataTable.Gruppe{idx_ndcn}, DataTable.sample_name{idx_ndcn}];
    end
...
[DataTable] = correctCellNumberForBeadCounts(DataTable,[opt.modelStates,'lin_pos','lin_neg']);
...
function [DataTable_corrected] = correctCellNumberForBeadCounts(DataTable,compartments)
    ...
    DataTable_corrected.mature = DataTable_corrected.CD34negativ+DataTable_corrected.CD45nichtdim+DataTable_corrected.lin_pos;
    ...
    ID4 = ~cellfun('isempty', regexp(DataTable_corrected.Properties.VariableNames,'lin_neg','once'));
    ID=ID1+ID2+ID3+ID4;
    for id=find(ID>0)
    ...
    if sum(sum([DataTable_corrected.HSC, DataTable_corrected.MPP, DataTable_corrected.MEP, DataTable_corrected.CMP, DataTable_corrected.GMP, DataTable_corrected.MLP],2)<=DataTable_corrected.lin_neg)<length(DataTable_corrected.HSC)
        disp('Bead corrected HSPC cell numbers do not add up to a sum > lineage negative cells in ')
        disp(num2str(sum(sum([DataTable_corrected.HSC, DataTable_corrected.MPP, DataTable_corrected.MEP, DataTable_corrected.CMP, DataTable_corrected.GMP, DataTable_corrected.MLP],2)>DataTable_corrected.lin_neg)))
        disp('cases!')
    end
